...
router.get('/user/:id', (req, res) => {
	...
	Idea.find({ 'author.id': req.params.id })
		.sort({ thumbs: 'desc' })
		.then((ideas) => {
...
router.get('/show/:id', async (req, res) => {
	...
	let toIdeas;
	let idea = await Idea.findOne({ _id: req.params.id }).populate({
		path: 'comments',
		options: { sort: { date: 'desc' } },
	});
	/* .then((idea) => { */
	if (idea) {
		toIdeas = await Idea.find({ topic: idea.topic }).where('_id').ne(idea._id).limit(3);
		/* .then((ideas) => {
						console.log(ideas);
						toIdeas = ideas;
					}); */
